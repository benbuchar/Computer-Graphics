...
function configureTexture(image) {
    ...
    var texSize = 512;
    // meowImage.src = "http://i.imgur.com/692QYLe.jpg";
    ...
    //     gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById("image"));
...
function init()
{
    ...
    // //var image = document.getElementById("basketBump");
    // gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
    ...
    */
    ...
    var ambientProductBuffer = gl.createBuffer();
    gl.bindBuffer( gl.ARRAY_BUFFER, ambientProductBuffer );
    gl.bufferData( gl.ARRAY_BUFFER, flatten(matAmbients), gl.STATIC_DRAW );

    var ambientProduct = gl.getAttribLocation( program, "ambientProduct" );
    gl.vertexAttribPointer( ambientProduct, 4, gl.FLOAT, false, 0, 0 );
    gl.enableVertexAttribArray( ambientProduct );
    
    var diffuseProductBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER,diffuseProductBuffer);
    gl.bufferData(gl.ARRAY_BUFFER,flatten(matDiffs),gl.STATIC_DRAW);
    
    var diffuseProduct = gl.getAttribLocation(program, "diffuseProduct");
    gl.vertexAttribPointer(diffuseProduct,4,gl.FLOAT,false,0,0);
    gl.enableVertexAttribArray(diffuseProduct);
    
    var specularProductBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER,specularProductBuffer);
    gl.bufferData(gl.ARRAY_BUFFER,flatten(matSpecs),gl.STATIC_DRAW);
    
    var specularProduct = gl.getAttribLocation(program,"specularProduct");
    gl.vertexAttribPointer(specularProduct,4,gl.FLOAT,false,0,0);
    gl.enableVertexAttribArray(specularProduct);
    ...
    var texSize = 512;
    var rawData = context.getImageData(0,0,512,512).data;
    ...
    var normals2 = new Uint8Array(3*texSize*texSize);

        for ( var i = 0; i < texSize; i++ )
            for ( var j = 0; j < texSize; j++ )
                for(var k =0; k<3; k++)
                    normals2[3*texSize*i+3*j+k] = 255*normalst[i][j][k];

    
    console.log(normals2);
    configureTexture(normals2);
...
{   
    ...
    // texCoordsArray.push(texCoord[1],texCoord[0],texCoord[2]);
    ...
    // texCoordsArray.push(texCoord[0],texCoord[1],texCoord[2]);
    // texCoordsArray.push(texCoord[0],texCoord[2],texCoord[3]);
    // texCoordsArray.push(texCoord[0],texCoord[1],texCoord[2]);    These configurations are 
    // texCoordsArray.push(texCoord[0],texCoord[2],texCoord[3]);    personal preference
    // texCoordsArray.push(texCoord[0],texCoord[1],texCoord[2]);
    // texCoordsArray.push(texCoord[0],texCoord[2],texCoord[3]);
    // texCoordsArray.push(texCoord[0],texCoord[1],texCoord[2]);
    // texCoordsArray.push(texCoord[0],texCoord[2],texCoord[3]);
    ...
    texCoordsArray.push(texCoord[1],texCoord[0],texCoord[2]);
    texCoordsArray.push(texCoord[2],texCoord[0],texCoord[1]);
    texCoordsArray.push(texCoord[1],texCoord[0],texCoord[2]);
    texCoordsArray.push(texCoord[2],texCoord[0],texCoord[1]);
    texCoordsArray.push(texCoord[1],texCoord[0],texCoord[2]);
    texCoordsArray.push(texCoord[2],texCoord[0],texCoord[1]);
    ...
    var frontNormal = calculateNormal(a,b,c);
    var leftNormal = calculateNormal(e,f,b);
    var rightNormal = calculateNormal(d,c,g);
    var backNormal = calculateNormal(h,g,f);
    var topNormal = calculateNormal(b,f,g);
    var bottomNormal = calculateNormal(d,h,e);
    ...
    if(uniform){
        ...
        var materialAmbient = vec4(vertexColors[Math.floor(Math.random()*6)] );
        var materialDiffuse = vec4(vertexColors[Math.floor(Math.random()*6)]);
        var materialSpecular = vec4( 1.0, 0.8, 0.0, 1.0 );
        materialShininess = Math.floor(Math.random()*100);
        ...
        for(var i=0; i<36; ++i){
            matAmbients.push(ambientProduct);
            matDiffs.push(diffuseProduct);
            matSpecs.push(specularProduct);
        }
    } else if(totalUniform){
        for(var i = 0; i<36; ++i){
            colors.push(tUColor);
        }
        var materialAmbient = vec4(0.75,0.75,0.75,1.0);
        //var materialAmbient = vec4(0.0,0.0,0.0,1.0);
        var materialDiffuse = tUColor;
        ...
        var materialDiffuse = vec4(vertexColors[Math.floor(Math.random()*6)]);
        var materialSpecular = vec4( 1.0, 0.8, 0.0, 1.0 );
        materialShininess = Math.random()*100;
        ...
        setMaterial(backTopRight);
        colors.push(vertexColors[bottomRight]);
        setMaterial(bottomRight);
        colors.push(vertexColors[backBottomRight]);
        setMaterial(backBottomRight);
        colors.push(vertexColors[backTopRight]);
        setMaterial(backTopRight);
...
function calculateNormal(a,b,c){
   ...
   // normal = normal + Math.random();
     var normal = vec3(normal);
    return normal;
}

function setMaterial(x){
    var materialDiffuse = vec4(vertexColors[x]);
    var materialSpecular = vec4(1.0, 0.8, 0.0, 1.0 );
    materialShininess = Math.floor(Math.random()*50)+50.0;
    var materialAmbient = vec4(vertexColors[x]);
    var ambientProduct = mult(lightAmbient, materialAmbient);
    var diffuseProduct = mult(lightDiffuse, materialDiffuse);
    var specularProduct = mult(lightSpecular, materialSpecular);
            matAmbients.push(ambientProduct);
